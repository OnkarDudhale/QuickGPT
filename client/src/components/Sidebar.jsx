...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    ...
    ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* new chat button */}
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        <input onChange={(e) => setSearch(e.target.value)} value={search} type="text" placeholder='Search Conversations'
          className='text-xs placehoder:text-gray-400 outline-none' />
      </div>
          ...
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase()
            ...
            .map((chat) => (
              ...
              dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
                ...
                <img onClick={e => toast.promise(deleteChat(e, chat._id),
                  { loading: 'deleting...' })} src={assets.bin_icon} className="lg:hidden group-hover:block w-4 cursor-pointer
                 not-dark:invert" alt='' />
              </div>
            ))
        }
      ...
      {/* community images */}
      <div onClick={() => { navigate('/community'); setIsMenuOpen(false); }} className='flex items-center gap-2 p-3 mt-3
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false); }} className='flex items-center gap-2 p-3 mt-3
      border border-gray-300 dark:border-white/15 rounded-md 
      cursor-pointer hover:scale-103 transition-all'>
      ...
      cursor-pointer group'>
        ...
        {user && <img onClick={logout} src={assets.logout_icon} className='h-5 cursor-pointer block lg:hidden not-dark:invert group-hover:block' />}
      </div>
